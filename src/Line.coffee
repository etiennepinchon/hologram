...
class exports.Line extends Entity
	...
	@define "start", entityLineProp("start", "start", null)
	@define "end", entityLineProp("end", "end", null)
	...
	_updateLineAttribute: ->
		attr = ""
		attr += "start: #{@_properties["start"]};" if @_properties["start"]
		attr += "end: #{@_properties["end"]};" if @_properties["end"]
