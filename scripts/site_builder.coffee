...
Config =
	bucket: "builds.hologram.cool"
	region: "us-east-1"
	input: "extras/builds.hologram.cool"
	output: "build/builds.hologram.cool"
...
main = ->
	exec "git config --local --get remote.origin.url", (err, output) ->
		...
		if output not in [
				"git@github.com:etiennepinchon/hologram.git",
				"https://github.com/etiennepinchon/hologram.git"]
